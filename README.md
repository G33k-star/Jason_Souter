# Jason Souter
Electrical Engineering student focused on embedded systems, digital signal processing, and intelligent sensing. I’m interested in building systems that connect physical measurements to real-time computation—from sensors and analog interfaces to signal processing, embedded software, and autonomous decision-making. My projects explore the intersection of embedded computing, sensing, instrumentation, signal processing, and electromagnetic systems, with an emphasis on understanding the complete measurement-to-decision pipeline.

## Areas of Interest
- Embedded Systems
- Digital Signal Processing
- Intelligent Sensing
- Embedded Instrumentation
- Sensor Fusion
- Real-Time Systems
- Electromagnetics & RF Systems
- Autonomous & Cyber-Physical Systems
- Signal Acquisition & Analysis
- Hardware/Software Co-Design

---

## Programming
- C / C++
- Python
- MATLAB

---

## Embedded Systems
- Microcontroller Programming
- Bare-Metal Development
- Real-Time Systems
- RTOS Fundamentals
- Embedded Linux
- Sensor Integration
- Data Acquisition
- Hardware/Software Co-Design
- Closed-Loop Control

### Platforms
- Arduino
- ESP32
- Raspberry Pi
- Linux-based Embedded Systems

---

## Digital Signal Processing
- Discrete-Time Signal Processing
- Digital Filtering
- Sampling & Data Acquisition
- Time-Domain Analysis
- Frequency-Domain Analysis
- FFT / Spectral Analysis
- Signal Conditioning
- Sensor Data Processing
- Feature Extraction
- Noise Analysis

### Python
- NumPy
- SciPy
- SymPy
- PySerial
- Jupyter

### MATLAB
- Signal Processing
- Numerical Analysis
- System Modeling
- Data Analysis
- Simulation

---

## Sensing & Instrumentation
- Sensor Interfacing
- Measurement Systems
- Data Acquisition
- Sensor Fusion
- Embedded Instrumentation
- Signal Conditioning
- Hardware Validation
- Experimental Measurement
- Real-Time Sensor Processing

---

## Communications & Interfaces
- UART
- SPI
- I²C
- Serial Communications
- Embedded Networking
- ROS / ROS2

---

## Systems Engineering
- Hardware/Software Co-Design
- Circuit Analysis
- System Integration
- Hardware Testing & Validation
- Computational Modeling
- Numerical Simulation
- Real-Time Data Processing
- Closed-Loop Systems

---

## Development Environment
- Linux
- Git / GitHub
- Visual Studio Code
- PlatformIO
- Arduino IDE
- Jupyter Notebook

---

## Current Research Interests
My current interests are centered on embedded sensing systems and the processing of physical measurements in real time, particularly:
- Embedded DSP Architectures
- Intelligent Sensing Systems
- Sensor Fusion & Multimodal Sensing
- Electromagnetic Sensing
- RF & Electromagnetic Systems
- Signal Acquisition & Processing
- Adaptive Cyber-Physical Systems
- Autonomous Sensing
- Multispectral & Multimodal Sensing
- Electronic Warfare & Counter-ISTAR Technologies

## Links
- [LinkedIn](www.linkedin.com/in/jason-souter-64baa4240)
- [ResearchGate](https://www.researchgate.net/profile/Jason-Souter?ev=hdr_xprf)
- [ORCID](https://orcid.org/0009-0002-9971-1584)
